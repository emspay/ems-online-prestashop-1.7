...
class emspayInstall {
	public function createTables() {
		...
		if ( !$db->Execute( '
		DROP TABLE IF EXISTS `'._DB_PREFIX_.'emspay`;
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'emspay` (
	...
	public function createOrderState() {
		if ( !Configuration::get( 'EMS_PAY_PENDING' ) ) {
			...
			Configuration::updateValue( 'EMS_PAY_PENDING', (int)$orderState->id );
		}

		if ( !Configuration::get( 'EMS_PAY_ERROR' ) ) {
			$orderState = new OrderState();
			$orderState->name = array();
			...
			Configuration::updateValue( 'EMS_PAY_ERROR', (int)$orderState->id );
		}
